...
public class Minecraft {
    public static void main(String[] args) throws IOException {
        ...
        if(!isFlat) {
            IntStream.rangeClosed(MIN, MAX)
                    .forEach(i -> timeNumArray[i] = calculateTimeSpent(blockNumArray, i, B, MIN, MAX));
            minTime = Arrays.stream(timeNumArray).parallel().filter(value -> value != 0).min().getAsInt();
            highestHeight = IntStream.range(0, timeNumArray.length)
                    .filter(i -> Objects.equals(timeNumArray[i], minTime))
                    .max().getAsInt();
        } else {
            minTime = 0;
            highestHeight = MAX;
    ...
    private static int calculateTimeSpent(int[] blockNumArray, final int HEIGHT,
                                          int remainBlocks, final int MIN, final int MAX) {
        if (MIN == MAX) {
            return 0;
        }
        int lostBlock = IntStream.rangeClosed(MIN, MAX)
                .filter(p -> (p < HEIGHT))
                .reduce(0, (result, val) -> result + ((HEIGHT - val) * blockNumArray[val]));
        int takeBlock = IntStream.rangeClosed(MIN, MAX)
                .filter(p -> (p > HEIGHT))
                .reduce(0, (result, val) -> result + ((val - HEIGHT) * blockNumArray[val]));
